...
while True:
  try:
      ...
      # Normalize the populationFitness values for each run
      ...
      # Create two legends
      # First legend for the runs (outside right)
      leg1 = ax.legend(title="Run", loc="center left", bbox_to_anchor=(1.02, 0.5), frameon=True)

      # Second legend for action types (bottom center)
      action_patches = [mpatches.Patch(color=color, label=action_types[action])
                        for action, color in action_colors.items()]
      leg2 = ax.legend(handles=action_patches, title="Action Type", loc="lower center", bbox_to_anchor=(0.5, -0.2), ncol=3, frameon=True)
      ...
      plt.tight_layout(rect=[0, 0.1, 0.8, 1])  # leave space on the right and bottom
